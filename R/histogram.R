             ...
             ...)
{
    ...
    ## The sequence of code in the next 3 lines is important, because 
    ## breaks needs to be evaluated before x is changed to a numeric
    ## from (possibly) a factor. types other than density are dumb when
    ## breakpoints are not equally spaced.
    ## There is a possibility of numerical error of course.
    
    if ((is.null(breaks) || length(unique(round(diff(breaks)))) != 1)
        && missing(type) && !missing(breaks))
        type <- "density"
    ...
    if(number.of.cond>0)
    {
        ...
        if(missing(xlim)) xlim <-
            if (is.null(breaks)) range( x[is.finite(x[!id.na])])
            else range( x[is.finite(x[!id.na])], breaks)
                ...
                if(any(id)) {
                    ...
                    if (!ylim.specified) {
                        ...
                        ylim[2] <- max(ylim[2],
                                       ...
                                       else if (type == "percent") 100*h$counts/length(x[id])
                                       else h$intensities)
                    }
    ...
    foo$panel.args.common <-
        if(subscripts) list(groups = groups, 
                            breaks = breaks,
                            type = type, ...)
        else list(breaks = breaks, type = type, ...)
