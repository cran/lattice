             ...
             ...)
{
    ...
    if(number.of.cond>0)
    {
        ...
        if(missing(ylim)) ylim <- range(y[is.finite(y[!id.na])])
    ...
    else
    {
        ...
        if(missing(ylim)) ylim <- extend.limits(range(y[is.finite(y[id])]))
    ...
    if(number.of.cond<1) 
        foo$panel.args <- 
            if(subscripts) list(x=x[id],y=y[id], subscripts=subscr[id])
            else list(x=x[id],y=y[id])
